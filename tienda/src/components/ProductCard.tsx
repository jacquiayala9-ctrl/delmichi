...
export default function ProductCard({ id, name, price, category, imageUrl }: ProductCardProps) {
  ...
  return (
    <div className="group relative flex flex-col bg-[#0c0514] border border-border-violet rounded-none overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-glow">
      ...
      {/* Content */}
      <div className="p-5 flex flex-col flex-grow z-20 relative bg-gradient-to-t from-[#09040e] to-[#0c0514]">
        <span className="text-accent-violet text-[0.65rem] uppercase tracking-[0.15em] mb-1.5 font-semibold">
          {category}
        </span>
        <Link href={`/producto/${id}`} className="block mb-2">
          <h3 className="font-heading text-lg text-foreground hover:text-accent-violet transition-colors line-clamp-2">
            {name}
          </h3>
        </Link>
